
const gymRoutes = {

    gymRegister:'/gym/gym_register',
    gymLogin:'/gym/gym_login',
    gymOtpVerify:'/gym/gym_otp_verify',
    gymOtpResend:'/gym/resend_otp',
    gymLogout:'/gym/logout',
    editGymSubscription:'/gym/edit_gym_subscription',
    fetchGymSubscription:`/gym/fetch_gym_subscription`,
    forgotPassword:'/gym/forgot_password',
    verifyForgotPassword:'/gym/verify_forgot',
    updataPasswordForgot:'/gym/update_password',
    resendForgotOtp:'/gym/resend_forgot_otp',
    fetchTrainers:`/gym/fetch_gym_trainers`,
    addTrainer:'/gym/add_gym_trainer',
    updateTrainer:`/gym/update_gym_trainer`,

}

export default gymRoutes