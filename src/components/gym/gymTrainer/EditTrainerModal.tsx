...
const EditTrainerModal = ({ editOpen,setEditOpen,refetch,selectedRow}) => {
  ...
  const handleSubmit = () => {
    ...
    console.log('formId ',selectedRow._id)
  ...
  return (
    ...
    <Dialog open={editOpen} onClose={() => setEditOpen(false)} fullWidth maxWidth="xs">
      <DialogTitle>Edit Trainer  <IconButton onClick={() => setEditOpen(false)} style={{float:'right'}}><CloseIcon color="primary"></CloseIcon></IconButton> </DialogTitle>
      <DialogContent>
        <Stack spacing={2} margin={2}>
        {formDatas.imageUrl ? (
            <img className="rounded-xl mx-auto object-cover w-1/2 " src={URL.createObjectURL(formDatas.imageUrl)} alt="Selected"  />
          ): (<img className="rounded-xl mx-auto object-cover w-1/2 " src={selectedRow?.imageUrl && selectedRow?.imageUrl} alt="Selected"  />)
